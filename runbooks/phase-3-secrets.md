...
```bash
# spec.encryptedData.db-password from secrets/demo-app-sealedsecret.yaml
<ciphertext> | base64 -d | xxd | head
...
committed set. `VAL` = the literal credential, `B64` = its base64 form:
...
```bash
kubectl delete -f secrets/demo-app-sealedsecret.yaml   # removes SealedSecret + owned Secret
helm uninstall sealed-secrets -n kube-system           # removes the controller + CRD-managed keys
```
