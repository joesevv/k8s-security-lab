...
One gap stated rather than buried: **the restructured two-job CI workflow has not yet run.** The signature the cluster admits today was produced by the earlier single-job version of the workflow. The `build-scan` / `sign` split that isolates `id-token: write`, the SHA-pinned actions and the cosign verify self-check are all committed but unproven, so `docs/evidence/phase-4-supply-chain/cosign-verify.txt` does not exist yet — see [§6.9 of the threat model](docs/threat-model.md#6-residual-risk-and-what-is-deliberately-out-of-scope). Next up: GitOps CD with ArgoCD — 2026-07-25.
